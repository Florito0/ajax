...
showMaxEntriesEditor(input) {
    // Supprimer tout éditeur existant pour éviter les conflits
    ...
    // HTML simplifié et fonctionnel
    editor.innerHTML = `
        <h4>Définir Max. Entrée par Qt. bouteilles</h4>
        <div class="editor-content">
            <div class="max-entries-inputs">
                </div>
        </div>
    ...
    // L'éditeur est ajouté au body pour éviter les problèmes de positionnement
    document.body.appendChild(editor);

    const inputsContainer = editor.querySelector('.max-entries-inputs');

    // Charger les données existantes de l'input vers l'éditeur
    try {
        ...
        if (Object.keys(maxEntries).length > 0) {
            for (const [qty, max] of Object.entries(maxEntries)) {
                this.addMaxEntryInput(inputsContainer, qty, max);
            }
        }
    } catch (e) { console.error("Données maxEntries invalides:", e); }

    // S'assurer qu'il y a au moins une ligne si aucune donnée n'est chargée
    ...
    // Positionner et afficher l'éditeur
    this.positionEditor(editor, input); // Utilise la fonction de positionnement que nous avons corrigée
    editor.style.display = 'flex';
}

positionEditor(editor, input) {
    // Avec position: absolute et le CSS ci-dessus, 
    // l'éditeur se positionnera automatiquement par rapport à l'input
    // On n'a plus besoin de calculs complexes
    
    // Vérification simple si l'éditeur sort de l'écran
    setTimeout(() => {
        const editorRect = editor.getBoundingClientRect();
        
        // Si l'éditeur sort à droite de l'écran
        if (editorRect.right > window.innerWidth - 20) {
            editor.style.left = 'auto';
            editor.style.right = '0';
        }
        
        // Si l'éditeur sort en bas de l'écran, le placer au-dessus
        if (editorRect.bottom > window.innerHeight - 20) {
            editor.style.top = 'auto';
            editor.style.bottom = 'calc(100% + 5px)';
        }
    }, 0);
}
...
saveMaxEntries(editor, targetInput) {
    ...
    editor.remove(); // Ferme l'éditeur après avoir sauvegardé
}
    ...
    // Construire correctement le tableau
    ...
    // MODIFICATION ICI : Choisir entre création et mise à jour
    let success;
    if (this.editingPrestationId) {
        // Mode édition : utiliser updateData
        success = await this.updateData(this.editingPrestationId, prestation);
    } else {
        // Mode création : utiliser saveData
